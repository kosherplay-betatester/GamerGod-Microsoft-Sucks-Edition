...
.DESCRIPTION
    ...
    machine at install time. GodMode itself changes nothing until you turn it on.
...
param(
    [string] $InstallRoot = (Join-Path $env:ProgramFiles 'GodMode'),
    [string] $SourceRoot,
    [switch] $NoPath
)
...
function Test-Prerequisites {
    ...
    if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'GodMode must be installed as Administrator. Right-click PowerShell and choose "Run as administrator".'
    }
    ...
    # every scheduling API GodMode relies on is 64-bit only in practice.
    if (-not [Environment]::Is64BitOperatingSystem) {
        throw 'GodMode requires a 64-bit version of Windows.'
    }
...
function Resolve-Source {
    ...
    foreach ($candidate in @(
            ...
            (Join-Path (Split-Path -Parent $here) 'src\GodMode.Cli\bin\Release\net10.0-windows\win-x64\publish'),
            (Join-Path (Split-Path -Parent $here) 'src\GodMode.Cli\bin\Release\net10.0-windows'),
            (Join-Path (Split-Path -Parent $here) 'src\GodMode.Cli\bin\Debug\net10.0-windows'))) {
        if (Test-Path (Join-Path $candidate 'godmode.exe')) {
            return (Resolve-Path $candidate).Path
        }
    }

    throw @'
Could not find godmode.exe to install.

Build it first:
    dotnet publish src/GodMode.Cli -c Release -r win-x64 --self-contained false
...
function Install-Service {
    ...
    if (-not (Test-Path $binary)) {
        ...
        Write-Skip 'GodMode will run as a command-line tool until the service ships'
        return
    }
    ...
    New-Service -Name $ServiceName `
        ...
        -DisplayName 'GodMode' `
        -Description 'Applies and reverses GodMode gaming optimisations, and restores the machine after a crash.' `
...
function Add-ToPath {
    ...
    Write-Step 'Adding GodMode to PATH'
...
Write-Host ''
Write-Host '  GodMode installer' -ForegroundColor White
Write-Host '  Nothing is optimised at install time. GodMode changes nothing until you turn it on.'
Write-Host ''
...
try {
    ...
    Write-Host '      godmode scan        ' -NoNewline -ForegroundColor Cyan
    Write-Host '- check this machine for things that cost you frames'
    Write-Host '      godmode topology    ' -NoNewline -ForegroundColor Cyan
    Write-Host '- see how your CPU will be partitioned'
    Write-Host ''
    Write-Host "  To remove GodMode completely, run Uninstall-GodMode.ps1. It reverts any"
