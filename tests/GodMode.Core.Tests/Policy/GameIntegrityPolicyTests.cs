...
namespace GodMode.Core.Tests.Policy;

/// <summary>
/// Charter Article I says GodMode will never break a game, and that the guarantee is
...
public sealed class GameIntegrityPolicyTests
{
    ...
    public void Contact_is_refused_by_default_even_when_provably_safe()
    {
        ...
        Assert.Equal(AntiCheatTier.UserMode, assessment.Tier);
        Assert.True(assessment.ContactIsConsidered);

        // No preference supplied - the default must be Never.
        var permit = GameIntegrityPolicy.Evaluate("Cyberpunk 2077", assessment);
    ...
    public void Contact_is_granted_only_when_opted_in_and_provably_safe()
    {
        ...
        Assert.Equal("Cyberpunk 2077", permit.Contact!.Title);
        Assert.Equal(AntiCheatTier.UserMode, permit.Contact.ObservedTier);
    }
