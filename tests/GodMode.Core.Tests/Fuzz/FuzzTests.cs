...
using GodMode.Core.Diagnostics;
...
namespace GodMode.Core.Tests.Fuzz;

/// <summary>
/// Randomised input across every entry point that reads data GodMode did not create.
///
/// <para>
...
public sealed class JournalResilienceTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), "godmode-tests", Guid.NewGuid().ToString("N"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "godmode-tests", Guid.NewGuid().ToString("N"), "none.journal");
