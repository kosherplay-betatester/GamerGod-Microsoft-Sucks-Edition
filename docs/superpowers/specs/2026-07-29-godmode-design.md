...
**The foundational invariant.** From inside a game process, a machine running GodMode
...
(BattlEye)** — GodMode never opens a handle to the game.
...
GodMode ever touching the game.
...
1. **Vanilla Diff.** For a title, capture loaded modules, handle count, its registry view,
   device list, and environment block with GodMode ON vs OFF. Assert identical.
...
3. **Auto-heal.** If a title that previously launched cleanly fails while GodMode is
   active, GodMode reverts, quarantines the profile, permanently marks the title
...
GodMode gets the same HUD without ever touching the game, because **measurement and
...
| **2 — RTSS bridge** | Publish GodMode metrics into RivaTuner's OSD via its shared-memory API | Exclusive fullscreen too | RTSS injects — that is the user's own pre-existing, consented install, never our code |
...
| **Live stutter attribution** — `⚠ hitch 340 ms — WSearch` appearing in real time | **The differentiator.** Every other overlay shows you a number went bad. GodMode tells you *what did it*, while you're still in the match |
| GodMode status line — active profile, `AMBIENT-ONLY`, detected AC tier | You always know exactly what is and isn't being done to your game |
...
| UI switch in GodMode and in the Playnite frontend | per-profile default |
...
GodMode's optimisations. The three are independent.
...
2. Battlefield 6 launches, passes its Javelin handshake, and plays identically with GodMode
