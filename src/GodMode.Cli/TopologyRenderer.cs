...
internal static class TopologyRenderer
{
    ...
    {
        WriteAccent(output, "  GodMode", ConsoleColor.Cyan);
    ...
    private static void RenderPlan(TextWriter output, CpuTopology topology)
    {
        ...
        if (topology.CanPartition)
        {
            ...
            foreach (var line in Wrap($"This machine can be partitioned. GodMode will {detail}.", Width))
            {
        ...
        else
        {
            ...
            output.WriteLine("  evict background work to. GodMode will apply every other ambient lever");
