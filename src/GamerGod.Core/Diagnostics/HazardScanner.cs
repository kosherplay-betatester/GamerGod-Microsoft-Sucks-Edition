...
///
/// <para>
/// This is the highest value-per-line feature in GamerGod and it changes nothing. On the
...
public static class HazardScanner
{
    ...
    /// They matter to GamerGod for two practical reasons: Memory Integrity refuses to load
    ...
    private static void ScanFailedDevices(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        ...
        foreach (var group in failures)
        {
            ...
            // source, well ahead of anything GamerGod could tune.
    ...
    private static void ScanHypervisors(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        ...
        if (thirdParty.Length > 0 && s.VirtualizationBasedSecurityRunning)
        {
            h.Add(new Hazard(
                ...
                + "start — one GamerGod did not cause and cannot fix.",
    ...
    private static void ScanSecurityPosture(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        ...
        // Doing so does not slow those games down, it stops them launching, so GamerGod's job
        ...
        if (!s.SecureBootEnabled || !s.TpmReady)
        {
            ...
            h.Add(new Hazard(
                ...
                "Enable Secure Boot and TPM 2.0 in firmware. GamerGod will never turn these "
        ...
        else
        {
            h.Add(new Hazard(
                ...
                + "machine. GamerGod will keep it that way — it never disables platform "
        ...
        if (s.VirtualizationBasedSecurityRunning && !s.MemoryIntegrityRunning)
        {
            h.Add(new Hazard(
                ...
                + "above from loading.",
                "Your call. GamerGod reports it and changes nothing either way."));
        }
    }
    ...
    private static void ScanNetwork(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        ...
        if (live.Length > 0 && live.All(a => a.IsWireless))
        {
            h.Add(new Hazard(
                ...
                + "that no amount of scheduling can remove — GamerGod can quiet the machine "
    ...
    private static void ScanRecovery(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        if (!s.SystemRestoreEnabled)
        {
            h.Add(new Hazard(
                ...
                "No restore points can be created. GamerGod's journal still guarantees that "
                ...
                + "missing, and GamerGod will decline to make those changes without it.",
    ...
    private static void ScanCapacity(MachineSnapshot s, ImmutableArray<Hazard>.Builder h)
    {
        if (s.PhysicalMemoryGigabytes is > 0 and <= 16)
        {
            h.Add(new Hazard(
                ...
                $"{s.PhysicalMemoryGigabytes} GB of memory — GamerGod helps more here",
        ...
        if (s.PerformanceDomainCount <= 1)
        {
            h.Add(new Hazard(
                ...
                "There is nowhere to evict background work to, so GamerGod will not partition "
                + "this processor. Every other lever still applies: efficiency-mode demotion, "
                + "service suppression, interrupt steering and power. GamerGod would rather say "
