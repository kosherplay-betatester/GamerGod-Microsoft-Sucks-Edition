...
namespace GamerGod.Core.Workloads;

/// <summary>What a companion tool does for GamerGod, so the offer can explain itself.</summary>
public enum CompanionRole
{
    ...
    /// <summary>An additional platform GamerGod can then optimise.</summary>
    Platform,
}

/// <summary>
/// An optional third-party tool GamerGod can integrate with.
///
/// <para>
/// Charter Article IX: build the conductor, not every instrument. GamerGod does not bundle,
...
/// Nothing here is required; GamerGod works without every one of them.
/// </para>
/// </summary>
public sealed record Companion
{
    ...
    /// <summary>What GamerGod gains. Shown verbatim in the offer, so it must be honest.</summary>
    public required string Benefit { get; init; }

    /// <summary>True when GamerGod is substantially less useful without it.</summary>
...
public static class CompanionCatalog
{
    ...
    [
        new()
        {
            ...
            Benefit = "Supplies the frametime data behind every measured claim GamerGod makes, "
                    + "and identifies which process is presenting frames without ever opening "
                    + "a handle to it. Without this, GamerGod cannot prove anything it says.",
        },
        new()
        {
            Name = "Playnite",
            ...
            Benefit = "Controller-navigable fullscreen library across Steam, Epic, GOG, Xbox "
                    + "and every emulator in GamerGod's catalogue. This is the console shell.",
        },
        new()
        {
            ...
            Benefit = "Precise frame limiting, used to sit the frame cap at the variable "
                    ...
                    + "where GamerGod's own non-injecting overlay cannot draw.",
            Caveat = "RTSS works by injecting into the game. That is its design, not "
                   + "GamerGod's, and GamerGod never asks it to inject into a title protected "
        ...
        new()
        {
            ...
            Benefit = "Google's official Android runtime for Windows, and the supported way to "
                    + "play Android titles on a PC. Once installed, GamerGod routes its virtual "
    ...
    /// <summary>
    /// Companions not yet present. The caller decides whether to offer them; GamerGod never
