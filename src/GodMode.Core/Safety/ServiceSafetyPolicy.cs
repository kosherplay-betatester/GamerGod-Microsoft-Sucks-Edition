...
public enum ProtectionReason
{
    ...
    /// <summary>Stopping it would disable GodMode's own recovery path.</summary>
    Recovery,
}

/// <summary>
/// The services GodMode will never stop, whatever a profile asks for.
///
/// <para>
...
public static class ServiceSafetyPolicy
{
    ...
    [
        ...
        Deny("MMCSS", ProtectionReason.Audio,
            ...
            + "stopping it degrades exactly the latency GodMode exists to improve."),
        ...
        Deny("amd3dvcache", ProtectionReason.Graphics,
            "AMD 3D V-Cache optimiser. It participates in the very scheduling GodMode tunes."),
        ...
        // ---- GodMode's own recovery path.
        Deny("EventLog", ProtectionReason.Recovery, "Event log. GodMode's diagnostics and post-crash trail."),
    ...
    /// <summary>
    /// True when GodMode must never stop this service.
    ///
    /// <para>
