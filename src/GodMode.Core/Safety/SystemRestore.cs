...
/// <summary>
/// Creates Windows System Restore points. Implemented in <c>GodMode.Windows</c> over the
...
public enum SafetyVerdict
{
    ...
    /// GodMode declines rather than making an unprotected change.
    /// </summary>
    Blocked,
}
...
/// <summary>
/// The second rung of GodMode's safety ladder.
///
/// <para>
/// The journal is the primary guarantee and covers the overwhelming majority of what
/// GodMode does — stopping a service or demoting a process cannot survive a reboot no
...
public static class SafetyGate
{
        ...
        bool userAcknowledgedNoRestorePoint = false)
    {
        ...
        return new SafetyDecision
        {
            ...
            Explanation =
                ...
                + "the risk. GodMode will not make an unprotected persistent change silently.",
        };
    }
    ...
    public static string DescribeRestorePoint(string sessionId, int changeCount) =>
        $"GodMode {sessionId[..Math.Min(8, sessionId.Length)]} — before {changeCount} persistent change(s)";
}
