...
///
/// <para>
/// This guards against the worst failure mode GodMode has. Confining every background
        ...
        CpuTopology topology)
    {
        ...
        else if (ambientMask.Count < MinimumAmbientProcessors)
        {
            violations.Add(new RoutingViolation(
                ...
                + "audio and GodMode's own watchdog remain responsive."));
        }
        ...
        if (!gameMask.IsEmpty && !ambientMask.IsEmpty)
        {
            if (gameMask.Group != ambientMask.Group)
            {
                violations.Add(new RoutingViolation(
                    "mask.group.mismatch",
                    "The game and ambient masks address different processor groups. GodMode "
